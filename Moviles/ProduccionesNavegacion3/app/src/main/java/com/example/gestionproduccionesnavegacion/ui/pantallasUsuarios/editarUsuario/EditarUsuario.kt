    ...
    private fun eventos(){
        with(binding){
            ...
            buttonProduccionesPendientes.setOnClickListener {
                val action = EditarUsuarioDirections.Companion.actionEditarUsuarioToListaTodasProducciones(args.id, false)
                findNavController().navigate(action)
            }

            buttonVistas.setOnClickListener {
                val action = EditarUsuarioDirections.Companion.actionEditarUsuarioToListaTodasProducciones(args.id, true)
                findNavController().navigate(action)
            }
            ...
            editarUsuarioViewModel.getProduccionesPendientes(args.id)
            editarUsuarioViewModel.getProduccionesVistas(args.id)
        }
    }
    ...
    private fun observacion(){
        editarUsuarioViewModel.state.observe(viewLifecycleOwner){state ->
            with(binding){
                ...
                password.editText?.setText(state.usuario.contrasenia)
                produccionesVistas.text = state.vistas.toString()
                produccionesPendientes.text = state.pendientes.toString()
